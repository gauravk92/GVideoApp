...
-(void)startCapturingScreenshots
{
    [_operationQueue addOperationWithBlock:^{
        ...
        UIWindow*   _window = [[[UIApplication sharedApplication] windows] objectAtIndex:1];
        NSDictionary *videoSettings = [[NSDictionary alloc] initWithObjectsAndKeys:
                                       AVVideoCodecH264, AVVideoCodecKey,
                                       [NSNumber numberWithFloat:1920], AVVideoWidthKey,
                                       [NSNumber numberWithFloat:1080], AVVideoHeightKey,
                                       nil];
...
CGImageRef UIGetScreenImage(CGFloat screen);

-(void)screenshot
{
    CGImageRef screen = (__bridge CGImageRef)[[[[UIApplication sharedApplication] windows] objectAtIndex:1] snapshotViewAfterScreenUpdates:YES].layer.contents;
